...
def generate_plots(N, mu, sigma2, S):
    ...
    Y = X + np.random.normal(mu, np.sqrt(sigma2), N)  # Add normal error with mean mu and variance sigma^2
    ...
    plt.plot(X, model.predict(X.reshape(-1, 1)), color="darkblue", label=f"Regression line: Y = {slope:.2f}X + {intercept:.2f}")
    plt.xlabel("X")
    plt.ylabel("Y")
